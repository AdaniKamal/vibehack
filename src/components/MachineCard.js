...
export default function MachineCard({ machine: m, onEdit, onDelete }) {
  const [hovered, setHovered] = React.useState(false);

  return React.createElement("div", {
    style: {
      background: "rgba(255,255,255,0.02)",
      border: `1px solid ${hovered ? "#00ff9d44" : "#21262d"}`,
    ...
    React.createElement("div", { style: { flex: 1, minWidth: 0 } },
      ...
      React.createElement("div", {
        ...
        React.createElement("span", { style: { fontWeight: "700", fontSize: "15px", color: "#fff" } }, m.name),
        ...
        React.createElement("span", { style: { color: "#555", fontSize: "10px" } }, m.platform),
        React.createElement("span", { style: { color: "#555", fontSize: "10px" } }, m.date)
      ),

      // Flags
      React.createElement("div", { style: { display: "flex", gap: "10px", marginBottom: "8px", flexWrap: "wrap" } },
        React.createElement("span", { style: { fontSize: "11px", color: m.userFlag ? "#00ff9d" : "#444" } },
          `${m.userFlag ? "✓" : "○"} user.txt`
        ),
        React.createElement("span", { style: { fontSize: "11px", color: m.rootFlag ? "#ff6b35" : "#444" } },
          `${m.rootFlag ? "✓" : "○"} root.txt`
        )
      ),
      ...
      m.notes && React.createElement("div", {
        style: { color: "#888", fontSize: "12px", marginBottom: "8px", fontStyle: "italic", borderLeft: "2px solid #21262d", paddingLeft: "10px" }
